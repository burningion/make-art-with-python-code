...
parser = argparse.ArgumentParser(description='Image tracer')
parser.add_argument('filename')
# add the drawing as an optional argument
parser.add_argument('-d', '--drawing', help='Filename of a drawing to load')
passedIn = parser.parse_args()

imageFile = pygame.image.load(passedIn.filename)
imageFile = pygame.transform.scale(imageFile, (800, 800))
...
# If we've got a drawing, load it, otherwise, new empty line
if passedIn.drawing:
    lines = pickle.load(open(passedIn.drawing, 'rb'))
else:
    lines = [Line()]
...
while running:
    ...
    # add the 's' key to save a file called 'lines.pkl'
    if key[pygame.K_s]:
